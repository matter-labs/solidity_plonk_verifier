...
library PairingsBn254 {
    ...
    function new_fr(uint256 fr) internal pure returns (uint256) {
        require(fr < r_mod, "MO"); /* !ERROR: Element is not in the field */
        return fr;
    }

    function inverse(uint256 fr) internal view returns (uint256) {
        require(fr != 0, "P2"); /* !ERROR: Can't invert zero */
    ...
    function pow(uint256 self, uint256 power) internal view returns (uint256) {
        ...
        require(success, "BG"); /* !ERROR: Error raising to power */
        return result[0];
    }
    ...
    function new_g1_checked(uint256 x, uint256 y) internal pure returns (G1Point memory) {
        ...
        // check encoding
        require(x < q_mod, "A"); /* !ERROR: 'x' is not in the field */
        require(y < q_mod, "EB"); /* !ERROR: 'y' is not in the field */
        // check on curve
        uint256 lhs = mulmod(y, y, q_mod); // y^2
        uint256 rhs = mulmod(x, x, q_mod); // x^2
        rhs = mulmod(rhs, x, q_mod); // x^3
        rhs = addmod(rhs, bn254_b_coeff, q_mod); // x^3 + b
        require(lhs == rhs, "ZR"); /* !ERROR: Point is not on the curve */
    ...
    function negate(G1Point memory self) internal pure {
        ...
        if (self.Y == 0) {
            require(self.X == 0, "O"); /* !ERROR: Invalid infinity point encoding */
            return;
        }
        ...
        internal view
    {
        if (p2.X == 0 && p2.Y == 0) {
            ...
            require(success, "3M"); /* !ERROR: Error adding curve points */
        }
    }
        ...
        internal view
    {
        if (p2.X == 0 && p2.Y == 0) {
            ...
            require(success, "PL"); /* !ERROR: Error subtracting curve points */
        }
    }
        ...
        internal view
    {
        ...
        require(success, "ET"); /* !ERROR: Error multiplying curve point */
    }

    function pairing(G1Point[] memory p1, G2Point[] memory p2)
        internal view returns (bool)
    {
        require(p1.length == p2.length, "BL"); /* !ERROR: Point arrays differ in length */
        ...
        require(success, "L"); /* !ERROR: Error computing pairing */
        return out[0] != 0;
    }
...
contract Plonk4VerifierWithAccessToDNext {
    ...
    function batch_evaluate_lagrange_poly_out_of_domain(
        ...
        require(vanishing_at_z != 0, "26"); /* !ERROR: Point z can't be in domain */
    ...
    function verify_at_z(
        ...
        require(lhs != 0, "WF"); /* !ERROR: Can't have point z in the domain */ // we can not check a polynomial relationship if point `z` is in the domain
    ...
    function verify_initial(
        ...
        require(proof.input_values.length == vk.num_inputs, "SX"); /* !ERROR: Incorrect number of input */
        require(vk.num_inputs >= 1, "D2"); /* !ERROR: Number of inputs must be >= 1 */
    ...
    function verify_recursive(
        ...
        require(recursive_input == proof.input_values[0], "2L"); /* !ERROR: Inconsistent input/proof */
    ...
    function reconstruct_recursive_public_input(
        ...
        require(length == individual_vks_inputs.length, "3X"); /* !ERROR: Arguments' lengths don't match */
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            require(index <= max_valid_index, "U"); /* !ERROR: Invalid index */
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            require(input < PairingsBn254.r_mod, "ZH"); /* !ERROR: Invalid input */
            assembly {
                mstore(base, input)
            }
            base += 32;
        ...
        for (uint256 i = 0; i < 16; i++) {
            ...
            require(input < MAX_LIMB_VALUE, "WJ"); /* !ERROR: Invalid subproof */
            assembly {
                mstore(base, input)
            }
            base += 32;
...
contract KeyedVerifier is Plonk4VerifierWithAccessToDNext {
    ...
    function deserialize_proof(
        ...
        require(serialized_proof.length == SERIALIZED_PROOF_LENGTH, "Y7"); /* !ERROR: Invalid proof length */
    ...
    function verify_serialized_proof(
        ...
        require(vk.num_inputs == public_inputs.length, "DS"); /* !ERROR: Invalid number of inputs */
    ...
    function verify_serialized_proof_with_recursion(
        ...
        VerificationKey memory vk = get_verification_key();
        require(vk.num_inputs == public_inputs.length, "XH"); /* !ERROR: Invalid length of input */
